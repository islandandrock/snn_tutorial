...
class NeuralNetwork:
    """
    Basic neural network, using same steps as the Tariq Rashid book 
    """
...
def main():
    # Test these learning rates for resulting accuracy
    rates = [.01, .1, .2, .3, .5, .8]
    results = []

    for rate in rates:
        results.append(test_rate(rate))
    print(results)
    
    # Create a plot to display accuracy / learning rate
    matplotlib.pyplot.plot(rates, results, marker='o')
